  ...
  const fetchNewsDetail = async () => {
    try {
      ...
      if (response.success) {
        ...
        if (response.data.event_image_url) {
          setEventImagePreview(`http://localhost:5000${response.data.event_image_url}`);
        }
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        if (selectedEventImage) {
            newsFormData.append('event_image', selectedEventImage);
        }
